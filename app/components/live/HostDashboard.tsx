...
interface HostDashboardProps {
  ...
  songs: Song[];
  onEndSession: () => void;
  onAddSongsById: (songIds: string[]) => void;
...
export default function HostDashboard({
  ...
  const handleSongPickerSelect = (songIds: string[]) => {
    onAddSongsById(songIds);
    setShowSongPicker(false);
  };

  const handlePlay = () => {
    onNavigateToSong(session.currentIndex ?? 0);
  };

  const connectedCount = peers.filter(p => p.status === 'connected').length;
  const existingSongIds = session.queue.map(q => q.songId);
    ...
    <>
      <div className="flex flex-col min-h-[calc(100vh-4rem)] px-6 pt-6 pb-4 max-w-2xl mx-auto w-full">
        ...
        {session.queue.length > 0 && (
          <button
            onClick={handlePlay}
            className="w-full py-3 mb-4 rounded-xl bg-[var(--accent)] text-white font-semibold hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2"
          >
      ...
      {showSongPicker && (
        <SongPicker
          ...
          onSelect={handleSongPickerSelect}
