...
export default function AnnotationOverlay({
  ...
  // Convert screen coords to base coords using SVG bounding rect (accounts for CSS transform)
  ...
  }, [activeTool, strokes, screenToBase, addPoint, erase, zoom, panX, panY, baseWidth, displaySize.width, clampPan]);
        ...
        <div className="flex items-center justify-between max-w-lg mx-auto">
          ...
          <div className="flex items-center gap-1.5">
            {DRAWING_COLORS.map((color) => (
              <button
                key={color}
                onClick={() => { setActiveColor(color as DrawingColor); setActiveTool('pen'); }}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => { if (strokes.length > 0) setShowClearConfirm(true); }}
              disabled={strokes.length === 0}
              title="Clear all"
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-[var(--background)] rounded-2xl p-6 max-w-sm w-full shadow-xl border border-[var(--border)]">
            <h3 className="text-lg font-bold text-[var(--foreground)] mb-2">Clear all annotations?</h3>
            <p className="text-sm text-[var(--muted)] mb-4">This will remove all annotation strokes.</p>
